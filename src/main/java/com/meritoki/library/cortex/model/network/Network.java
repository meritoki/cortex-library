...
public class Network extends Cortex {
	...
	public Network(com.meritoki.library.cortex.model.network.ColorType color, int x, int y) {
...
//				} else 
	...
	@JsonIgnore
	public void propagate(com.meritoki.library.cortex.model.network.ColorType type, Concept concept, boolean flag) {
//		logger.info("propogate(" + concept + ")");
		Level level = null;
		int size = this.getLevelList().size();
		for (int i = 0; i < size; i++) {
